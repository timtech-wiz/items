   ...
   @endif
    ...
    <div class="mt-6">
        <div class="flex justify-between">
            <div>
                <input type="search" wire:model.debounce.500ms="q" placeholder="Search" class="shadow appearance-none 
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        ...
                        @if(!$active)
                            <td class="border px-4 py-2">{{$item->status == 1 ? 'Active' : 'Not Active'}}</td>
                        @endif
    ...
    <x-jet-confirmation-modal wire:model="confirmItemDeletion">
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-3" wire:click="deleteItem({{$confirmItemDeletion}})" wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-jet-danger-button>
        </x-slot>
